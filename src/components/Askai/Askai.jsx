...
import { getResponseFromAIBackend, getSentiment, getEmail, getSummary, fe_getContent, fe_sendEmail } from "../../js/aiCalls";
...
*/}
  ...
  async function submitQuery() {
    ...
    setAiresponse('')
    setAiresponse2('')
    document.getElementById("vr_id").innerHTML = ''
    document.getElementById("vr_id_c").innerHTML = ''
    setLoadingShowHide1('block')
    //let getServiceResult = "Answer: Artificial intelligence VR, or AI, is technology that enables computers and machines to simulate human intelligence and problem-solving capabilities. AI, or artificial intelligence, is technology that allows computers and machines to mimic human intelligence and problem-solving abilities."
     
    let getServiceResult = await getResponseFromAIBackend(query).catch((e) => {
    //let getServiceResult = await getLLMResponseFromAIBackend(query).catch((e) => {
    ...
    if (getServiceResult) {
      ...
      setAiresponse(getServiceResult['apiresponse']);
      //setAiresponse();
      document.getElementById("vr_id").innerHTML = getServiceResult1;
    } else {
      setBdisable(false);
      setAiresponse("Something went wrong pls try again.");
    }
  ...
  return (
    <div>
      <Grid fullWidth className="portal-top-grid ">
        ...
        <Column sm={4} md={8} lg={16}>
          <div style={{ fontWeight: "bold" }}>Ask Sales Assistant Using LLM</div>
        </Column>
